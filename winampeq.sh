...
#
#### To check mixer type: alsamixer -D equal
#
set_equalizer_curve() {
  curve="${*}"
  ctl=0
...
profile="${1:-flat}"
case "${profile}" in
list) echo -e "List of EQ presets:\n flat | classical | club | dance | headphones | bass | treble\n large_hall | live | party | pop | reggae | rock | ska | soft_rock | soft | techno" ;;
